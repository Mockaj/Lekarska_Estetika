...
$warning_box = $database->general("name", "warning_box")->fetch();
 if (isset($_GET['save'])) { 
        ...
        $id = $myPost['id'];
        unset($myPost['id']);
        $database->general->where("id", $id)->update($myPost);
        Header('Location:top-lista?save-ok');
              ...
              <div class="col">
                <div class="card card-small mb-4">
                  <div class="card-header border-bottom">
                    <h6 class="m-0" style="float:left;">Box s upozorněním</h6>
                  </div>
                  <div class="card-body p-10 pb-3 text-left">


                      
                        <?php foreach ($database->general->where("part",0) as $zaznam) { 
                        ?>

                            <form method="post" action="?upravit&save">
                                <input type="hidden" name="id" value="<?= $zaznam['id'] ?>">
                                <div class="form-group">
                              <strong style="float:left;">Obsah</strong><br style="clear:both;">
                              <textarea name="value" id="editor1" class="form-control"><?= $zaznam['value'] ?></textarea>
                            </div>
                                <button type="submit" class="btn btn-success" style="margin-left:10px;">Uložit</button>
                            </form>
